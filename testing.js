(function(){
            var pushcrewConfig = {
            companyName: "CSGORollTEST",
            hash: "4844bdf7eee4650e82f2cffcbf3e9664",
            origin: "https",
            subdomainFlag: "0",
            ...
            migrationDetails: 0,
            style: {
                url: "https://pushcrew.com/https-v4/css/https-v4.css",
                type: "safari",
                iconUrl: "https://cdn.pushcrew.com/img/logos/4844bdf7eee4650e82f2cffcbf3e9664/4251ae09-44b9-4154-b4b7-4375d54547fa.png",
                allowTextChildWindow: "",
                offerTextChildWindow: "Get updates and offers from CSGORoll by subscribing to push notifications.",
                ...
                chromeTitle: "ct1",
                chromeSubtitle: "ct2",
                safariTitle: "The website “CSGORoll” would like to send you push notifications.",
                safariSubtitle: "Notifications can be turned off anytime from browser settings.",
                allowText: "Allow",
                dontAllowText: "Don\'t Allow",
                brandingType: 0,
                brandingText: "Please Click on \'Allow\' in the permission box to get instant notifications from our website. No Email Required.",
                isMobileEnabled: 0,
                safariMobileTitle: "",
                mobileChickletPosition: "mobile-chicklet-right"
            }
        };;(function() {
var detectPrivateMode, globalConfig, storageUtil, jquery, pushSubscription, manageSubscription, customizejQuery, initAPI, api, httpsMain, src_https_without_tracker;
...
manageSubscription = function ($pushcrew, globalConfig) {
...
api = function ($pushcrew, globalConfig) {
  $pushcrew(function () {
    ...
    globalConfig._pushcrew.removeSubscriberFromSegment = function (segment, callback) {
      ...
      window._pushcrewDebuggingQueue.push('addSubscriberToSegment execution complete');
    };
...
httpsMain = function ($pushcrew, globalConfig, initAPI) {
  $pushcrew(document).ready(function () {
    var storageUtil, manageSubscription, pushSubscription, cssUrl, head, link, prefetchImagesList, secondsTrigger, promise, timestamp;
    storageUtil = globalConfig.pushAPI.storageUtil;
    manageSubscription = globalConfig.pushAPI.manageSubscription;
    pushSubscription = globalConfig.pushAPI.pushSubscription;
    ...
    function notificationClickBranding() {
      ...
      var httpsBranding;
      var isBrandingPossible = pushcrewConfig.pcBranding === 1 && window.location.search.indexOf('pushcrew_powered') !== -1;
      var isUnsubscribePossible = pushcrewConfig.unsubscribeNCFlag === 1 && (window.location.search.indexOf('_p_c') !== -1 || window.location.search.indexOf('pushcrew_powered') !== -1);
      if (!(isBrandingPossible || isUnsubscribePossible)) {
        return;
      }
      if (/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
        httpsBranding = '<div class="pushcrew-https-branding-modal-mobile-click-box">  ' + '    <a href="javascript:void(0)" class="js-pushcrew-https-branding-modal-mobile-click-close pushcrew-https-branding-mobile-click-close">\xD7</a>';
        if (isUnsubscribePossible) {
          httpsBranding += '    <a href="javascript:void(0);" class="pushcrew-unsubscribe">Unsubscribe from Notifications</a>';
        }
        if (isBrandingPossible) {
          httpsBranding += '<div class="pushcrew-https-branding-modal-mobile-click-body"><a href="https://pushcrew.com/?ref=' + pushcrewConfig.companyName + '" target="_blank" style="text-decoration: none;">' + '<span>Notifications Powered By </span><img src="https://pushcrew.com/assets/images/pushcrew-white.png" width=85/>' + '</a></div>';
        }
        httpsBranding += '</div>';
      } else {
        httpsBranding = '<div class="pushcrew-https-branding-modal-clicked-box">  ' + '    <a href="javascript:void(0)" class="js-pushcrew-https-branding-modal-clicked-close pushcrew-https-branding-clicked-close">\xD7</a>';
        if (isUnsubscribePossible) {
          httpsBranding += '    <a href="javascript:void(0);" class="pushcrew-unsubscribe">Unsubscribe from Notifications</a>';
        }
        if (isBrandingPossible) {
          httpsBranding += '  <div class="pushcrew-https-branding-modal-clicked-body"><a href="https://pushcrew.com/?ref=' + pushcrewConfig.companyName + '" target="_blank" style="text-decoration: none;">' + '<span>Notifications Powered By </span><img src="https://pushcrew.com/assets/images/pushcrew-white.png" alt="pushcrew" width=85/>' + '</a></div>';
        }
        httpsBranding += '</div>';
      }
      if (httpsBranding) {
        $pushcrew('body').append(httpsBranding);
      }
      //User click on Unsubscribe from Notificaitons
    ...
    var isChromeSupported = 'chrome' in window && 'serviceWorker' in navigator && globalConfig.getBrowser().version >= 42;
    var isFirefoxSupported = /Firefox[\/\s](\d+\.\d+)/.test(navigator.userAgent) && 'serviceWorker' in navigator && globalConfig.getBrowser().version >= 44;
    var androidVersion = parseFloat(globalConfig.getAndroidVersion());
    if (/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
      isFirefoxSupported = false;
    }
    if (!isChromeSupported && !isFirefoxSupported) {
      window._pushcrewDebuggingQueue.push('Push notification is not supported by Chrome and Firefox');
      globalConfig._pushcrew.apiFailureReasons = { reasons: ['BROWSER_NOT_SUPPORTED'] };
    ...
    detectPrivateMode.detectPrivateMode(function (is_private) {
      if (!is_private) {
        ...
        link.href = cssUrl;
        prefetchImagesList = [
          'https://pushcrew.com/assets/images/pushcrew-white.png',
        ...
        link.onload = function () {
          ...
          // Notification Click Branding
          notificationClickBranding();
          ...
          if (pushSubscription.browserType === 'chrome') {
            window._pushcrewDebuggingQueue.push('Broswer type is Chrome');
            ...
            promise.then(function (response) {
              if (response === false && pushSubscription.canPushBeEnabled === true) {
                manageSubscription.syncUnsubscribedUsers().then(function () {
                  if (storageUtil.getFromStorage('wingify_push_subscription_status') === 'unsubscribed' && Notification.permission === 'granted') {
          ...
          } else if (pushSubscription.browserType === 'safari' && pushcrewConfig.safariWebsitePushId !== '') {
            window._pushcrewDebuggingQueue.push('Broswer type is Chrome Safari');
            pushSubscription.websiteSafariPushId = pushcrewConfig.safariWebsitePushId;
            if (globalConfig.pushcrew.noTrigger || pushcrewConfig.triggerAutomaticOptIn === 0) {
              return;
            }
            setTimeout(function () {
          ...
          } else if (pushSubscription.browserType === 'firefox') {
            //check for browser firefox
            window._pushcrewDebuggingQueue.push('Broswer type is Chrome Firefox');
...
}(customizejQuery, globalConfig, initAPI);
src_https_without_tracker = function () {
  window._pushcrewDebuggingQueue = window._pushcrewDebuggingQueue || [];
}();
}());